...
YuWanCard 是一个基于 BaseLib 框架开发的 Slay the Spire 2 模组，实现了 33 张无色卡牌、6 个能力效果、12 个遗物、1 个先古之民、1 个怪物、1 个事件和 1 个无尽模式 modifier。

**内容概览**：
- 33 张无色卡牌（包含猪主题卡牌、雨落狂流之暗、梭哈、顺手牵羊、拔你网线等）
- 6 个能力（猪疑惑、雨落狂流、猪破产、猪脑过载、你是猪、猪冲锋力量）
- 12 个遗物（七咒之戒、10 年孤竹、昊天锤、重瞳、至尊骨、傲慢猪、嫉妒猪、暴怒猪、懒惰猪、贪婪猪、暴食猪、色欲猪）
- 1 个先古之民（猪猪）
- 1 个怪物（杀手精英）
- 1 个事件（铁匠铺）
- 1 个无尽模式 modifier
...
5. **卡牌设计**：
   ...
   - **能力提示显示**：使用 `ExtraHoverTips` 属性显示卡
...
- BaseLib 使用指南：[BaseLib 使用指南.md](BaseLib使用指南.md)
