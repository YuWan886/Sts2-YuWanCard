...
- **Joker 槽位系统**：独立于普通遗物的 Joker 遗物装备栏
- **连击乘数系统**：基于回合内打出的卡牌累积连击，伤害获得倍率加成
- **卡牌修饰器系统**：独立于升级/附魔的第三层卡牌强化（铝箔/全息/多彩/负片）
...
4. [卡牌修饰器系统](#4-卡牌修饰器系统)
5. [利息经济系统](#5-利息经济系统)
6. [Balatro 卡牌](#6-balatro-卡牌)
7. [Joker 遗物](#7-joker-遗物)
8. [配套遗物](#8-配套遗物)
9. [Balatro 能力](#9-balatro-能力)
10. [UI 与交互](#10-ui-与交互)
11. [文件结构](#11-文件结构)
12. [设计文档对照](#12-设计文档对照)
...
| 2 | 初始解锁 | 修改器激活即可用 |
| 3 | 初始解锁 | 修改器激活即可用 |
| 4 | 击杀第 1 个 Boss | 完成第一幕后自动解锁 |
| 5 | 击杀第 2 个 Boss | 完成第二幕后自动解锁 |
| 6 | 获得「负片小丑」 | 仅在拥有 NegativeJoker 时生效，同时解锁第 4、5 槽（若未解锁） |

### 2.2 Joker 获取方式

- **精英/Boss 掉落**：击杀精英有 25% 概率、击杀 Boss 有 100% 概率额外掉落一个 Joker 遗物（独立于普通遗物奖励，不重复获得已拥有的 Joker）
- **空白兑换券**：BlankVoucher 遗物获得时从 3 个随机 Joker 中选择 1 个获得

### 2.3 Joker 的装备与存储

Joker 遗物继承 `YuWanJokerRelicModel`，核心行为：
1. **获得时**（`AfterObtained`）→ 自动尝试装备到空闲槽位
2. 若槽位已满 → 存入 **Joker 背包**（以 `|` 分隔的字符串列表持久化）
...
独立于升级和附魔的**第三层卡牌强化**。每张 `YuWanCardModel` 卡牌最多拥有 **1 个修饰器**。修饰器数据保存在 `YUWANCARD_Edition` 和 `YUWANCARD_FoilApplied` 两个 SavedProperty 中。

### 4.2 修饰器类型

| 修饰器 | 枚举值 | 效果 | 视觉风格 |
|--------|--------|------|---------|
| 铝箔 (Foil) | `BalatroCardEdition.Foil` | 所有正数值 +20%（向下取整，最小+1），仅应用一次 | 银色边框 |
| 全息 (Holographic) | `BalatroCardEdition.Holographic` | 战斗中卡牌费用 -1（最低 0） | 蓝紫渐变 |
| 多彩 (Polychrome) | `BalatroCardEdition.Polychrome` | 打出时卡牌效果额外触发 1 次 | 彩虹流动 |
| 负片 (Negative) | `BalatroCardEdition.Negative` | 标记为有修饰器（触发联动效果），不计入牌组数量 | 紫色反转 |
...
- **保留**：升级、Deserialize 时保留修饰器（反序列化后若 Foil 未应用则重新应用）
- **复制**：卡牌复制时修饰器一起复制
...
- **非猪角色限制**：非猪角色不会在卡池中看到 Investment、CompoundInterest、Dividend、Bankruptcy、Inflation（这些卡牌依赖猪的经济机制）
...
全部 12 个 Joker 遗物继承 `YuWanJokerRelicModel`，注册在 `SharedRelicPool`。它们**不出现在普通遗物池和商店中**（`IsAllowed => false`, `IsAllowedInShops => false`），仅通过精英/Boss 奖励掉落和 BlankVoucher 获取。
...
- **GreedJoker**：在 `AfterCardPlayed` 中计数攻击牌，每 3 张触发一次，金幣获得量 = 5 × 贪婪小丑数量
- **GluttonyJoker**：同上，每 4 张技能牌回复 3 × 暴食小丑数量点生命
- **MirrorJoker**：在 `ModifyCardPlayCount` 中检测 `LastCardTypeThisTurn == card.Type`，额外触发次数 = 镜像小丑数量
- **MiserJoker**：在 `ModifyDamageAdditive` 中计算手牌中 0 费牌数量，加法加成 = 0 费牌数 × 守财小丑数量
- **CollectorJoker**：在 `AfterPlayerTurnStart` 中统计牌组 Rare/Ancient 牌，能量 = floor(RareAncientCount / 5) × 收藏家数量
- **GamblerJoker**：在 `AfterCardPlayed` 中检测连击 ≥ 5，随机目标 8~20 伤害
- **PolychromeJoker**：在 `ModifyCardPlayCount` 中对有修饰器的卡牌额外 +1 触发
- **NegativeJoker**：获得时自动解锁第 4、5、6 槽
- **LegendJoker**：`GetLegendBonus()` 返回 `CardsPlayedThisTurn × 0.2 × Count`
- **HolographicJoker**：`AfterPlayerTurnStart` 中复制 `PreviousTurnFirstCard` 加入手牌
- **BankerJoker**：利息计算中 +3 × 数量
- **InvestorJoker**：在 `AfterItemPurchased` 中返还 `floor(goldSpent × 0.2 × Count)`
- **Blueprint**：配套遗物，复制最右侧 Joker 槽位效果（将最右侧 Joker ID 重复加入生效列表）

---

## 8. 配套遗物

8 个普通遗物继承 `BalatroRelicModel`，注册在 `SharedRelicPool`。仅在 Balatro 模式激活时出现（`IsAllowed` 检查 `BalatroModifier.IsActive`）。
...
- 显示内容：
  ...
  - **始终**：`JOKER [GREED] [GLUTTONY] [-] [🔒] [🔒]`（装备槽位状态）
- 交互：
  - 可拖动（支持鼠标和触摸）
  - 自动限制在视口范围内
...
│   ├── BalatroCardEditionHelper.cs           # 修饰器应用辅助类
│   ├── BalatroCardPool.cs                    # Balatro 卡牌池（共享无色）
│   ├── BalatroJokerRelicModel.cs             # Joker 遗物抽象基类
│   └── BalatroRelicModel.cs                  # Balatro 配套遗物抽象基类
│
├── Modifiers/
│   └── BalatroModifier.cs                    # 修改器主体（~857 行，所有核心逻辑）
│
├── Cards/Balatro/                            
│
├── Relics/Jokers/                         
...
│   ├── BalatroTopBarModifierFilterPatch.cs   # TopBar 修饰器日志
│   └── BalatroDebugPatches.cs                # 调试日志补丁
│
└── UI/
    └── NBalatroHudPanel.cs                   # 可拖动 HUD 面板（连击 + Joker 显示）
```
...
| 文件 | 内容 |
|------|------|
| `localization/eng/balatro_ui.json` | 修饰器类型名称和描述（英文） |
| `localization/zhs/balatro_ui.json` | 修饰器类型名称和描述（中文） |
| `localization/eng/modifiers.json` | Balatro 修改器名称和描述 |
| `localization/eng/relics.json` | 全部 20 个遗物本地化 |
| `localization/eng/cards.json` | 全部 13 张卡牌本地化 |
| `localization/eng/powers.json` | 全部 3 个能力本地化 |

### 图片资源

| 文件 | 用途 |
|------|------|
| `images/modifiers/balatro.png` | 修改器图标 + TopBar 按钮图标 |
...
- 5 大核心子系统全部实现（Joker 槽位、连击乘数、卡牌修饰器、利息经济、消耗品卡牌）
- 12 个 Joker 遗物全部实现（效果与设计一致）
- 8 个配套遗物全部实现
- 13 张 Balatro 卡牌全部实现
- 3 个 Balatro 能力全部实现
- 角色选择 Tickbox 和 UI 面板（简化版）
