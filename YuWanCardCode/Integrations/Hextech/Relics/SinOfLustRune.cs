...
public sealed class SinOfLustRune : HextechPigRuneBase
{
    ...
    public override async Task AfterPowerAmountChanged(PowerModel power, decimal amount, Creature? applier, CardModel? cardSource)
    {
        ...
        if (!_spreadUsedThisTurn && power.Owner.Side == MegaCrit.Sts2.Core.Combat.CombatSide.Enemy && Owner.Creature.CombatState != null)
        {
            ...
            {
                await PowerCmd.Apply(ModelDb.GetById<PowerModel>(power.Id).ToMutable(), enemy, amount, Owner.Creature, cardSource);
            }
        }
        ...
        _debuffsAppliedThisTurn = 0;
        Flash();
        await PowerCmd.Apply<DexterityPower>(Owner.Creature, DynamicVars.Dexterity.BaseValue, Owner.Creature, cardSource);
    }
}
